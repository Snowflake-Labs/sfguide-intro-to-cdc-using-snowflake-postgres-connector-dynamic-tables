...
def display_charts(data):
    ...
    total_items_data = data_pd.groupby(["TRANSACTION_DATE", "TRANSACTION_CATEGORY"]).agg(
        {"QUANTITY": "sum"}).reset_index().rename(columns={"QUANTITY": "TOTAL_ITEMS"})
    ...
    with col1:
        ...
        ).properties(title="Daily Transactions").interactive()

        st.altair_chart(histogram, use_container_width=True)

    card_data = data_pd.groupby("TRANSACTION_CARD").agg({"TRANSACTION_ID": "count"}).reset_index().rename(
    ...
    with col2:
        ...
        ).properties(title="Transactions by Card Type").interactive()
        st.altair_chart(card_chart, use_container_width=True)

    empty_space()

    col3, col4 = st.columns(2)
    purchases_by_category = data_pd.groupby("PRODUCT_CATEGORY").agg({"TRANSACTION_ID": "count"}).reset_index().rename(
        columns={"TRANSACTION_ID": "PURCHASE_COUNT"})
    with col3:
        ...
        ).properties(title="Transactions by Category").interactive()
        st.altair_chart(category_chart, use_container_width=True)
    merchant_data = data_pd.groupby("MERCHANT_NAME").agg(
        {"TRANSACTION_ID": "count", "TOTAL_PRICE": "sum"}).reset_index().rename(
        columns={"TRANSACTION_ID": "TRANSACTION_COUNT"})
    ...
    with col4:
        ...
        ).properties(title="Transactions by Merchant",
                     padding={"left": 10, "right": 10, "top": 10, "bottom": 10}).configure_axis(labelFontSize=12,
...
def display_promotions(data, customer_spending, spend_status):
    ...
    st.sidebar.subheader("Promotions")
    st.sidebar.info(
        f"**Exclusive promotion for our {spend_status.lower()}:** **\${credit}** credit for customers who spend **\${required_spend}** at **{highest_transaction_merchant}** over a period of 6 months.")
